...
def calculate_loss(feat_result, feat_content, feat_style, indices, content_weight, moment_weight=1.0, cmd_weight=0.1):
    ...
    loss_style = loss_remd + moment_weight * loss_moment + cmd_weight * loss_cmd
    style_weight = 1.0 + moment_weight + cmd_weight
    loss_total = (content_weight * loss_content + loss_style) / (content_weight + style_weight)
    return loss_total

def calculate_loss1(feat_result, feat_content, feat_style, indices, content_weight, rmd_weight=2.0, cmd_weight=1.0, coff_weight=1e-4):
    # spatial feature extract
    num_locations = 1024
    spatial_result, spatial_content = spatial_feature_extract(feat_result, feat_content, indices[0][:num_locations], indices[1][:num_locations])
    loss_content = content_loss(spatial_result, spatial_content)
    ...
    loss_remd = cal_rmd_loss(spatial_result[:, :feat_max, :, :], spatial_style[:, :feat_max, :, :])
    loss_cmd=cal_cmd_loss(spatial_result[:,:3,:,:], spatial_style[:,:3,:,:],k=1)
    loss_coff=cal_coeff_loss(spatial_result[:,:3,:,:], spatial_style[:,:3,:,:])
    # palette matching
    content_weight_frac = 1./max(content_weight,1.)
    
    loss_style=rmd_weight*loss_remd+cmd_weight*loss_cmd+coff_weight*loss_coff

    style_weight = rmd_weight+cmd_weight+coff_weight
    loss_total = (content_weight * loss_content + loss_style) / (content_weight + style_weight)
    return loss_total
