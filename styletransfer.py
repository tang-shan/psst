...
import cfg
from sample import get_prob_map
...
class Vgg16_Extractor(nn.Module):
    ...
    def forward_samples_hypercolumn(self, X, prob, samps=100):
        feat = self.forward(X)
        
        samples = min(samps,X.shape[-1]*X.shape[-2])
        
        index=np.array([i for i in range(X.shape[-2]*X.shape[-1])])
...
def cmd_loss(X,Y):
    ...
    weights=[1,1/8,1/16]
    for i in range(2,4):
        m_ys.append((Y - uy).pow(i).mean(dim=0))
        m_xs.append((X - ux).pow(i).mean(dim=0))
    loss+=torch.sqrt(nn.MSELoss()(ux, uy))
    for i in range(2):
        loss+=weights[i+1]*torch.sqrt(nn.MSELoss()(m_xs[i], m_ys[i]))
    return loss

def cal_rmd_loss(X, Y, cos_d=True):
    d = X.shape[1]
...
def cal_cmd_loss(X,Y,k=2):
    loss = 0.
    X = X.squeeze().t()
    Y = Y.squeeze().t()
    ux = torch.mean(X, 0, keepdim=True)
    uy = torch.mean(Y, 0, keepdim=True)
    m_ys = list()
    m_xs= list()
    weights=[]
    for i in range(1,k):
        weights.append(1/(32*i))
    loss+=torch.abs(ux-uy).mean()
    for i in range(2,k+1):
        m_ys.append((Y - uy).pow(i).mean(dim=0))
        m_xs.append((X - ux).pow(i).mean(dim=0))
    for i in range(k-1):
        loss+=weights[i]*torch.abs(m_xs[i]- m_ys[i]).mean()
    return loss

def cal_coeff_loss(X,Y):
    loss = 0.
    X = X.squeeze().t()
    Y = Y.squeeze().t()
    mu_x = torch.mean(X, 0, keepdim=True)
    ...
    X_cov = torch.mm(X_c.t(), X_c) / (X.shape[0] - 1)
    Y_cov = torch.mm(Y_c.t(), Y_c) / (Y.shape[0] - 1)
    sigma_x=(X - mu_x).pow(2).mean(dim=0)
    sigma_y=(Y - mu_y).pow(2).mean(dim=0)
    X_coeff=X_cov/(sigma_x+1e-6)
    Y_coeff=Y_cov/(sigma_y+1e-6)
    loss+=torch.abs(X_coeff-Y_coeff).mean()
    return loss
    

def cal_moment_loss(X, Y, moments=[1,2]):
    loss = 0.
    X = X.squeeze().t()
    Y = Y.squeeze().t()
...
def calculate_loss(stylized,feat_result, feat_content, feat_style, indices, content_weight, moment_weight=1.0, cmd_weight=0.1,tv_weight=0.1):
    ...
    loss_cmd = cal_cmd_loss(spatial_result[:,:3,:,:], spatial_style[:,:3,:,:],k=5)
    loss_tv=total_variation_loss(stylized)
    # print(f'Style: {loss_style.item():.3f}, Content: {loss_content.item():.3f}')
    loss_style = loss_remd + moment_weight * loss_moment + cmd_weight * loss_cmd + tv_weight * loss_tv
    style_weight = 1.0 + moment_weight + cmd_weight
...
def optimize(result, content, style, scale, content_weight, lr, extractor):
    ...
    prob=np.array(get_prob_map(content, style))
    ...
    for i in range(5):
        with torch.no_grad():
            ...
            feat_e = extractor.forward_samples_hypercolumn(style, prob, samps=1000)
    ...
    for it in range(opt_iter):
        ...
        loss = calculate_loss(stylized,feat_result, feat_content, feat_style, [xx, xy], content_weight)
        loss.backward()
        optimizer.step()
    return stylized

#the transfer network is modified from strotss
...
if __name__ == "__main__":
    ...
    if cfg.resize_to < 2**8:
        print("Resulution too low.")
        exit(1)
    
    content_pil, style_pil = pil_loader(cfg.content_path), pil_loader(cfg.style_path)
    content_weight = cfg.weight * 16.0
        
    device = cfg.device
        
    start = time()
    result = strotss(pil_resize_long_edge_to(content_pil, cfg.resize_to), 
                             pil_resize_long_edge_to(style_pil, cfg.resize_to), content_weight, device, cfg.ospace)
    result.save(cfg.output)
